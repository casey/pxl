use std::{
    collections::BTreeMap, path::{Path, PathBuf},
...
impl Module {
            ...
            .map_err(|io_error| (io_error, module_path))?
        {
            ...
            let kind = if metadata.is_dir() {
                ...
                match extension {
                    ...
                    | Some("tif") | Some("tiff") | Some("tga") | Some("bmp") | Some("ico")
                    | Some("hdr") | Some("pbm") | Some("pam") | Some("ppm") | Some("pgm") => {
                        ResourceKind::Image
                    }
    ...
    pub fn tokens(self) -> Tokens {
      let children = self.children.into_iter().map(|(identifier, child)| child.tokens(identifier));
